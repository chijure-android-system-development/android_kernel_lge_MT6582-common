        ...
        u8 *data, int buf_bytes)
{
    ...
    if ( card->cid.manfid == 0x15 ) {  // Samsung
        cmd->arg = 0x0c7810000;
    } else if (card->cid.manfid == 0x90 ) { // SK Hynix
        cmd->arg = 0x00006600;
    } else if (card->cid.manfid == 0x11 ) { // Toshiba
        //cmd->arg = 0xffffffff;
...
int mmc_ffu_install(struct mmc_card *card)
{
    ...
    if (!FFU_FEATURES(ext_csd[EXT_CSD_FFU_FEATURES])) {
        pr_err("FFU: %s: MODE_OPERATION_CODES is not supported :\n",
                mmc_hostname(card->host));
        /* restart the eMMC */
