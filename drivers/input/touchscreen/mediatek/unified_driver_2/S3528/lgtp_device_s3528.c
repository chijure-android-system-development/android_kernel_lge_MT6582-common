...
#define LPWG_CONTROL_REG		0x1B; /*f12_info.ctrl_reg_addr[18];*/
...
static ssize_t store_incoming_call(struct i2c_client *client, const char *buf, size_t count)
{
	...
	{
		//Controlling the touch part is taken care in InitRegister function
	}
	else if(reply == INCOMING_CALL_IDLE )//call status is idle && idle status. ==> sensitivity is normal(pen)
	{
		buffer[0] = 0x19;
		buffer[1] = 0x1e;
...
static int S3528_InitRegister(struct i2c_client *client)
{
	...
	//Touch is reset for waking from LPWG mode to normal mode so do setting here
...
static int S3528_InterruptHandler(struct i2c_client *client,TouchReadData *pData)
{
	...
	if (regIntStatus == INTERRUPT_MASK_NONE) {
		...
		if (readFingerCnt > 0) {
			...
			for (i = 0; i < readFingerCnt; i++) {
				if (regFingerData[i][0] & 0x03) {
					...
					//TOUCH_LOG("S3528_InterruptHandler( qcover = %d )\n", qcover);
					if(qcover == 1)
		...
		else if (lpwgStatus & 0x2)	/* TCI-2 : Multi-Tap */
		{
			...
			tci_control(client, REPORT_MODE_CTRL, 0);		// change to normal for overtap check
