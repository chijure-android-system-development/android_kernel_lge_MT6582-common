...
int BattThermistorConverTemp(int Res)
{
	...
	int RES1 = 0, RES2 = 0;
	int TBatt_Value = -200, TMP1 = 0, TMP2 = 0;
	int i_max = ARRAY_SIZE(Batt_Temperature_Table) - 1;

	if (Res >= Batt_Temperature_Table[0].TemperatureR) {
		TBatt_Value = Batt_Temperature_Table[0].BatteryTemp;
	} else if (Res <= Batt_Temperature_Table[i_max].TemperatureR) {
		TBatt_Value = Batt_Temperature_Table[i_max].BatteryTemp;
	} else {
		RES1 = Batt_Temperature_Table[0].TemperatureR;
		TMP1 = Batt_Temperature_Table[0].BatteryTemp;
...
kal_int32 fgauge_read_capacity_by_v(kal_int32 voltage)
{
	...
	if (voltage > (profile_p + 0)->voltage) {
		return FG_SOC_MAX;	/* battery capacity, not dod */
	}
	if (voltage < (profile_p + saddles - 1)->voltage) {
		return FG_SOC_MIN;	/* battery capacity, not dod */
	}
#else	/* MTK Original */
	if (voltage > (profile_p + 0)->voltage) {
...
#ifdef CONFIG_LGE_PM_SOC_SCALE
		{
			kal_int32 scaled_dod;
			scaled_dod = ((temp_profile_p + i)->percentage * 100)
				/ SOC_RESCALING_FACTOR;
			scaled_dod = scaled_dod - (100 - SOC_RESCALING_FACTOR);
			(temp_profile_p + i)->percentage = scaled_dod;
		}
#endif
#if 0
		(temp_profile_p + i)->voltage = temp_v_2 +
...
#ifdef CONFIG_LGE_PM_BATTERY_PROFILE
	bm_print(BM_LOG_CRTI,"Changed profile to <DOD,Voltage> at %d = <%d,%d>\r\n",
		temperature,(temp_profile_p + 0)->percentage,(temp_profile_p + 0)->voltage);
#else
...
#ifdef CONFIG_LGE_PM_BATTERY_PROFILE
	bm_print(BM_LOG_CRTI, "Changed r-table profile to <Rbat,VBAT> at %d = <%d,%d>\r\n",
		temperature, (temp_profile_p + 0)->resistance,(temp_profile_p + 0)->voltage);
#else
...
void dod_init(void)
{
	...
	if (abs(gFG_capacity_by_v-gFG_capacity_by_v_init) > HW_SW_SOC_DIFFERENCE) {
	    gFG_capacity_by_v = gFG_capacity_by_v_init;
	...
	if (g_rtc_fg_soc != 0)
	{
		kal_bool use_rtc_fg_soc = KAL_FALSE;

		/* If soc difference is lower that CUST_POWERON_DELTA_CAPACITY_TOLRANCE,
		  use previous soc */
		if (abs(g_rtc_fg_soc-gFG_capacity_by_v) < CUST_POWERON_DELTA_CAPACITY_TOLRANCE) {
			use_rtc_fg_soc = KAL_TRUE;

			/* If current soc is too low to use previous one, update soc */
			if (gFG_capacity_by_v <= CUST_POWERON_LOW_CAPACITY_TOLRANCE) {
				use_rtc_fg_soc = KAL_FALSE;
			}
		}

		/* If Reboot occured, use previous soc */
		if (g_boot_reason == BR_WDT ||
		...
		if (use_rtc_fg_soc == KAL_TRUE) {
			bm_print(BM_LOG_CRTI, "[FGADC] use previous soc. prev = %d, cur = %d\n",
				g_rtc_fg_soc, gFG_capacity_by_v);
...
void oam_init(void)
{
	...
	{
		kal_int32 v = g_booting_vbat;
		kal_int32 r = fgauge_read_r_bat_by_v(gFG_voltage);
		kal_int32 i = (((gFG_voltage - v) * 1000) * 10) / r;	/* 0.1mA */

		if (i > 8000){
			i = 8000;
		}
		/* If charger boot-up the device, HW OCV must be bumped-up.
		  So we reduce current to half */
		if (g_boot_reason == BR_USB)
			i = i / 2;

		/* compensate battery voltage */
		v = v + mtk_imp_tracking(v, i, 5);

		bm_print(BM_LOG_CRTI, "[oam_init] g_booting_vbat compensated : %dmV -> %dmV\n",
			g_booting_vbat, v);
...
#ifdef CONFIG_MTK_EXTERNAL_CHARGER_SUPPORT
		// if charger wake-up the device, ZCV is bumped up. so we use vbat instead ZCV
		else if (g_boot_reason == BR_USB)
		{
			bm_print(BM_LOG_CRTI,
				"[oam_init] fg_vbat=(%d), vbat=(%d), set fg_vat as vat (BR_USB)\n",
				gFG_voltage,g_booting_vbat);
...
#ifdef CONFIG_LGE_PM_SOC_SCALE
	if (oam_d_1 < FG_DOD_MIN)
		oam_d_1 = FG_DOD_MIN;
	if (oam_d_1 > FG_DOD_MAX)
		oam_d_1 = FG_DOD_MAX;
#else	/* MTK Original */
	if (oam_d_1 < 0)
		oam_d_1 = 0;
...
#ifdef CONFIG_LGE_PM_SOC_SCALE
	if (oam_d_2 < FG_DOD_MIN)
		oam_d_2 = FG_DOD_MIN;
	if (oam_d_2 > FG_DOD_MAX)
		oam_d_2 = FG_DOD_MAX;
#else	/* MTK Original */
	if (oam_d_2 < 0)
		oam_d_2 = 0;
...
#ifdef CONFIG_LGE_PM_SOC_SCALE
	if (oam_d_3 < FG_DOD_MIN)
		oam_d_3 = FG_DOD_MIN;
	if (oam_d_3 > FG_DOD_MAX)
		oam_d_3 = FG_DOD_MAX;
#else	/* MTK Original */
	if (oam_d_3 < 0)
		oam_d_3 = 0;
...
#ifdef CONFIG_LGE_PM_SOC_SCALE
	/* If battery voltage is less then mimimum profile voltage, then return FG_DOD_MAX */
	/* If battery voltage is greater then maximum profile voltage, then return FG_DOD_MIN */
	if (voltage > (profile_p + 0)->voltage) {
		return FG_DOD_MIN;
	}
	if (voltage < (profile_p + saddles - 1)->voltage) {
...
#ifdef CONFIG_LGE_PM_SOC_SCALE
	if (gFG_DOD0 > FG_DOD_MAX) {
		gFG_DOD0 = FG_DOD_MAX;
		bm_print(BM_LOG_FULL, "[fgauge_update_dod] gFG_DOD0 set to %d, gFG_columb=%d\r\n",
			 FG_DOD_MAX, gFG_columb);
	} else if (gFG_DOD0 < FG_DOD_MIN) {
		gFG_DOD0 = FG_DOD_MIN;
		bm_print(BM_LOG_FULL, "[fgauge_update_dod] gFG_DOD0 set to %d, gFG_columb=%d\r\n",
			 FG_DOD_MIN, gFG_columb);
	} else {
	}
#else	/* MTK Original */
	if (gFG_DOD0 > 100) {
...
#endif
#ifdef CONFIG_LGE_PM_SOC_SCALE
	if (FG_dod_1 > FG_DOD_MAX) {
		FG_dod_1 = FG_DOD_MAX;
		bm_print(BM_LOG_FULL, "[fgauge_update_dod] FG_dod_1 set to %d, gFG_columb=%d\r\n",
			 FG_DOD_MAX, gFG_columb);
	} else if (FG_dod_1 < FG_DOD_MIN) {
		FG_dod_1 = FG_DOD_MIN;
		bm_print(BM_LOG_FULL, "[fgauge_update_dod] FG_dod_1 set to %d, gFG_columb=%d\r\n",
			 FG_DOD_MIN, gFG_columb);
	} else {
...
/* ============================================================ // */
#ifdef CONFIG_LGE_PM_BATTERY_PROFILE
static int battery_meter_set_profile(void)
{
#ifdef CONFIG_LGE_PM_BATTERY_ID
	BATT_ID id = lge_get_battery_id();

	switch (id) {
	case BATT_ID_DS2704_N:
		SET_BATTERY_PROFILE(DS2704_N);
		break;
	case BATT_ID_DS2704_L:
		SET_BATTERY_PROFILE(DS2704_L);
		break;
	case BATT_ID_DS2704_C:
		SET_BATTERY_PROFILE(DS2704_C);
		break;
	case BATT_ID_ISL6296_N:
		SET_BATTERY_PROFILE(ISL6296_N);
		break;
	case BATT_ID_ISL6296_L:
		SET_BATTERY_PROFILE(ISL6296_L);
		break;
	case BATT_ID_ISL6296_C:
		SET_BATTERY_PROFILE(ISL6296_C);
		break;
	case BATT_ID_RA4301_VC0:
		SET_BATTERY_PROFILE(RA4301_VC0);
		break;
	case BATT_ID_RA4301_VC1:
		SET_BATTERY_PROFILE(RA4301_VC1);
		break;
	case BATT_ID_RA4301_VC2:
		SET_BATTERY_PROFILE(RA4301_VC2);
		break;
	case BATT_ID_SW3800_VC0:
		SET_BATTERY_PROFILE(SW3800_VC0);
		break;
	case BATT_ID_SW3800_VC1:
		SET_BATTERY_PROFILE(SW3800_VC1);
		break;
	case BATT_ID_SW3800_VC2:
		SET_BATTERY_PROFILE(SW3800_VC2);
		break;
	default:
		bm_print(BM_LOG_CRTI, "[battery_meter_probe] Invalid battery attached.\n");
		break;
	}
	bm_print(BM_LOG_CRTI, "[battery_meter_probe] battery_id = %d\n", id);
#endif
	return 0;
}
#endif
...
static int battery_meter_resume(struct platform_device *dev)
{
	...
	if (_g_bat_sleep_total_time > 3600) {	/* 1hr */
		if (hw_ocv_after_sleep < g_hw_ocv_before_sleep) {
			...
			oam_car_2 = 0;
#ifdef CONFIG_LGE_PM
			/* update soc immediatly */
			if (oam_d0 > oam_d_5)
				reset_parameter_dod_full(100 - oam_d0);
#endif
		} else {
