	...
	switch (cmd)
	{
	   ...
	   case ECS_IOCTL_RESET:
		...
		case ECS_IOCTL_GET_DELAY:
			...
			delay[3] = (int)akmd_delay * 1000000;
		...
		case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:			
			...
			osensor_data->values[0] = sensor_data[9] * CONVERT_O/CONVERT_O_DIV;
			osensor_data->values[1] = sensor_data[10] * CONVERT_O/CONVERT_O_DIV;
			osensor_data->values[2] = sensor_data[11] * CONVERT_O/CONVERT_O_DIV;
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				osensor_data->values[0] = sensor_data[9] * CONVERT_O;
				osensor_data->values[1] = sensor_data[10] * CONVERT_O;
				osensor_data->values[2] = sensor_data[11] * CONVERT_O;
				osensor_data->status = sensor_data[8];
				osensor_data->value_divide = CONVERT_O_DIV;
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	AKMDBG("%s: OK\n", __func__);
    printk(KERN_ERR " AKM09911 akm09911_probe: done \n");
