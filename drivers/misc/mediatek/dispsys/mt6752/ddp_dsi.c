...
#define DSI_POLLREG32(cmdq, addr,mask,value)  \
	do{\
		cmdqRecPoll(cmdq,((unsigned long)addr)&0x1fffffff, value, mask);\
	}while(0);
...
unsigned long long  cmd_done_times=0;
unsigned long long  te_done_times=0;
#endif
...
//		for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
		{
			DSI_POLLREG32(cmdq, 0x1401200C, 0x80000000, 0x0);
		}
		return;
	}
...
//	unsigned int fmod = 30;//Fmod = 30KHz by default
	...
	for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	{
	    ...
	    MIPITX_OUTREGBIT(MIPITX_DSI_DATA_LANE0_REG,DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE0,RG_DSI_LNT0_RT_CODE,((lnt >> 16) & 0xf));
	     ...
	     DISPMSG("PLL config: Int=0x%x, LNT=0x%x,clk=0x%x,lan3=0x%x,lan2=0x%x,lan1=0x%x,lan0=0x%x\n",
...
#else
        //building error; //should considing 64bit value
#endif
		// step 1
...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	hs_trail_m=1;
	hs_trail_n= (dsi_params->HS_TRAIL == 0) ? NS_TO_CYCLE(((hs_trail_m * 0x4) + 0x60), cycle_time) : dsi_params->HS_TRAIL;
	...
	// --------------------------------------------------------------
	timcon1.DA_HS_EXIT  = (dsi_params->DA_HS_EXIT == 0) ? NS_TO_CYCLE(0xC8, cycle_time) : dsi_params->DA_HS_EXIT;

	timcon2.CLK_TRAIL	= ((dsi_params->CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64, cycle_time) : dsi_params->CLK_TRAIL) + 0x01;
	...
	timcon3.CLK_HS_EXIT= (dsi_params->CLK_HS_EXIT == 0) ? NS_TO_CYCLE(0xC8, cycle_time) : dsi_params->CLK_HS_EXIT;
	timcon3.CLK_HS_POST= (dsi_params->CLK_HS_POST == 0) ? NS_TO_CYCLE((0x3c + 0x34 * ui), cycle_time) : dsi_params->CLK_HS_POST;
...
int ddp_dsi_power_on(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (!s_isDsiPowerOn)
	{
		...
		DSI_Reset(module, NULL);//add by caozhengguang  //ZHIBIN1216
			if(module == DISP_MODULE_DSIDUAL)
	{
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	{	
		...
		for(i = 0; i < 3; i++)
		{
	        ...
	        if(dsi_i==0) // DSI0
	        {
		        ret = cmdqRecPoll(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[i])+0xc), 0x1, 0x00000001);
		        ret = cmdqRecWrite(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[i])+0xc), 0x0, 0x00000001);
	        }
#if 0
			else // DSI1
			{			
			        ret = cmdqRecPoll(cmdq_trigger_handle, 0x1401c00c, 0x1, 0x00000001);
			        ret = cmdqRecWrite(cmdq_trigger_handle, 0x1401c00c, 0x0, 0x00000001);
			}
#endif
	        // 2. save RX data 
	        if(hSlot)
	        {
	        	cmdqRecBackupRegisterToSlot(cmdq_trigger_handle, hSlot, i, 0x14012074);
	...
	else if(state == CMDQ_STOP_VDO_MODE)
	{
		...
		if(i==0) // DSI0/DUAL
		{   //polling vm done
		    //ret = cmdqRecPoll(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[i])+0xc), 0, 0x00000010);
			//polling dsi busy
			ret = cmdqRecPoll(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[i])+0xc), 0, 0x80000000);
#if 0
			i = DSI_MODULE_END(module);
			if(i==1) // DUAL
			{
...
int DSI_set_roi(int x,int y)
{
	...
	DSI_Send_ROI(DISP_MODULE_DSI0, NULL,  x, y, _dsi_context[0].lcm_width, _dsi_context[0].lcm_height);
...
int DSI_test_roi(int x,int y)
{
	...
	else
	{
		...
		DSI_Send_ROI(DISP_MODULE_DSI0, NULL,  0, 0, _dsi_context[0].lcm_width, _dsi_context[0].lcm_height);
	}

	return ret;
}

int DSI_test_force_te(DISP_MODULE_ENUM module)
{
	int ret=0,r=0;
    int i=0;
	if (_dsi_is_video_mode(module))
	...
	printk("DSI_test_force_te1\n");
	for(i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++)
	{
        r= wait_event_interruptible_timeout(_dsi_wait_ext_te[i], !(DSI_REG[i]->DSI_INTSTA.TE_RDY), HZ/50);
        if (0 == r)
        {
	        DISPERR("dsi wait not busy timeout\n");
        }
	...
	unsigned int data_array[16];
	data_array[0]= 0x00000020;
	DSI_set_cmdq(module, NULL, data_array, 1, 1);
	printk("DSI_test_force_te2\n");

	for(i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++)
	{
        r= wait_event_interruptible_timeout(_dsi_wait_ext_te[i], !(DSI_REG[i]->DSI_INTSTA.TE_RDY), HZ/5);
        if (0 == r)
        {
        	printk("wait TE timeout!\n");
			break;
        }
	}
	...
	if(r<=0)
    {
        printk("no force on TE\n");
		return -1;
    }
	unsigned long long temp_te = te_done_times;
	unsigned long long temp_cmd = cmd_done_times;
    unsigned long long diff = temp_te-temp_cmd;
	if(diff>0)
	{
        printk("no force on TE %lld\n",diff);
		return -1;
    }

    printk("force on TE %lld\n",diff);

/*	

	printk("DSI_test_force_te3\n");
	for(i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++)
	{
        r= wait_event_interruptible_timeout(_dsi_wait_ext_te[i], !(DSI_REG[i]->DSI_INTSTA.EXT_TE), HZ/50);
        if (0 == r)
        {
        	printk("wait TE timeout!\n");
			break;
	...
	printk("DSI_test_force_te4\n");

	for(i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++)
	{   DSI_OUTREGBIT(NULL, DSI_RACK_REG,DSI_REG[i]->DSI_RACK,DSI_RACK,1);
        r = wait_event_interruptible_timeout(_dsi_cmd_done_wait_queue[i], !(DSI_REG[i]->DSI_INTSTA.CMD_DONE), HZ/50);
        if (0 == r)
        {   
            printk("wait cmd done timeout!\n");
			ret =-1;
			break;
        }
	}*/
...
unsigned int DSI_WaitForNotBusyTimeout(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	int i = 0;
	unsigned int count = 0;
	unsigned int tmp = 0;
...
//		for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	...
	for(i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++)
	{
        r = wait_event_interruptible_timeout(_dsi_cmd_done_wait_queue[i], !(DSI_REG[i]->DSI_INTSTA.BUSY), WAIT_TIMEOUT);
        if (0 == r)
        {
	        DISPERR("dsi wait not busy timeout\n");
			ret = 1;
