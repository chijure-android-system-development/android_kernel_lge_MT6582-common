                    ...
                    void * handle)  // ourput setting
{
    ...
    DDPDBG("RDMAConfig idx %d, mode %d, address %lx, inputformat %s, pitch %u, width %u, height %u\n",
    ...
    {
    	unsigned int fifolen = width*3*3/16/2;
	DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_UNDERFLOW_EN, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 1);	 
	if(0x200<fifolen)
		DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, 0x200); //FHD:304, HD:203, QHD:151
	else
        	DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, idx*DISP_RDMA_INDEX_OFFSET+ DISP_REG_RDMA_FIFO_CON, fifolen); //FHD:304, HD:203, QHD:151	       
    }  
#endif
    return 0;
}
