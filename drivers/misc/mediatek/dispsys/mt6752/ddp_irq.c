...
void disp_dump_emi_status(void)
{
    ...
    DDPMSG("dump emi status, infra_base_va=0x%lx, emi_base_va=0x%lx,\n", infra_base_va, emi_base_va); 
    printk("0x10203000: ");
    for(i=0;i<0x158;i+=4)
    {
        printk("0x%x, ", *(volatile unsigned int*)(emi_base_va+i));
        if(i%32==0 && i!=0)
...
irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
    	...
    	if(irq==dispsys_irq[DISP_REG_DSI0])
	{
        ...
        else if(irq==dispsys_irq[DISP_REG_OVL0] || irq==dispsys_irq[DISP_REG_OVL1])
		{
                ...
                if(reg_val&0x1e0)
                {
                    MMProfileLogEx(ddp_mmp_get_events()->ddp_abnormal_irq, MMProfileFlagPulse, (index<<16)|reg_val, module);
                }
        	}
        else if(irq==dispsys_irq[DISP_REG_WDMA0] || irq==dispsys_irq[DISP_REG_WDMA1])
		{
                ...
                if(reg_val&0x2)
                {
                    MMProfileLogEx(ddp_mmp_get_events()->ddp_abnormal_irq, MMProfileFlagPulse, (cnt_wdma_underflow[index]<<24)|(index<<16)|reg_val, module);
                }
        }
        else if(irq==dispsys_irq[DISP_REG_RDMA0] || irq==dispsys_irq[DISP_REG_RDMA1])
		{
                ...
                if(reg_val&(1<<1))
                {
                         ...
                         rdma_start_irq_cnt[0] > rdma_done_irq_cnt[0]+3)
                      {
                          ...
                          MMProfileLogEx(ddp_mmp_get_events()->ddp_abnormal_irq, MMProfileFlagPulse, 0, DISP_MODULE_UNKNOWN);
                ...
                if(reg_val&(1<<4))
                {
                      DDPMSG("rdma%d, pix(%d,%d,%d,%d) \n",
                          index,
                          DISP_REG_GET(DISP_REG_RDMA_IN_P_CNT+DISP_RDMA_INDEX_OFFSET*index),
                          DISP_REG_GET(DISP_REG_RDMA_IN_LINE_CNT+DISP_RDMA_INDEX_OFFSET*index),
                          DISP_REG_GET(DISP_REG_RDMA_OUT_P_CNT+DISP_RDMA_INDEX_OFFSET*index),
                ...
                if(reg_val&0x18)
                {
                    MMProfileLogEx(ddp_mmp_get_events()->ddp_abnormal_irq, MMProfileFlagPulse, (rdma_underflow_irq_cnt[index]<<24)|(index<<16)|reg_val, module);
                }
        }
        ...
        else if(irq==dispsys_irq[DISP_REG_CONFIG])  // MMSYS error intr
		{
            ...
            DDPERR("MFG APB error cnt=0x%x, MJC APB error cnt=0x%x \n", 
                DISP_REG_GET(DISP_REG_CONFIG_MFG_APB_TX_CON)&0x3ffff,
                DISP_REG_GET(DISP_REG_CONFIG_MJC_APB_TX_CON)&0x1ffff);
