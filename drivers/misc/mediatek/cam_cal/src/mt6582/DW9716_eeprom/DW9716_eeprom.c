...
int iWriteEEPROM_dw9714(u16 a_u2Addr  , u32 a_u4Bytes, u8 * puDataInBytes)
{
    int  i4RetValue = 0;
    u32 u4Index = 0;
...
int iReadEEPROM_dw9714(u16 a_u2Addr, u32 ui4_length, u8 * a_puBuff)
{
    int  i4RetValue = 0;
...
int iWriteData_dw9714(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int  i4RetValue = 0;
...
int iReadDataFromDw9716(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
...
int iReadData_dw9714(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int  i4RetValue = 0;
   int  i4ResidueDataLength;
   u32 u4IncOffset = 0;
...
//   EEPROMDB("[S24EEPORM] iReadData \n" );
   ...
   do 
   {
       ...
       {
           i4RetValue = iReadEEPROM_dw9714((u16)u4CurrentOffset, 8, pBuff);
           if (i4RetValue != 0)
           {
                EEPROMDB("[EEPROM] I2C iReadData failed!! \n");
                return -1;
       ...
       {
           i4RetValue = iReadEEPROM_dw9714((u16)u4CurrentOffset, i4ResidueDataLength, pBuff);
           if (i4RetValue != 0)
           {
                EEPROMDB("[EEPROM] I2C iReadData failed!! \n");
                return -1;
...
#endif//COMMON_CAM_CAL_DRV
{
    ...
    switch(a_u4Command)
    {
...
#endif            
            i4RetValue = iWriteData_dw9714((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
#ifdef EEPROMGETDLT_DEBUG
        ...
        case CAM_CALIOC_G_READ:
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000d9716) && (EEPROM_Address[0]==0x0))
            {
            ...
            else if(ptempbuf->u4Offset == 0x0028C32a)
            {
            ...
            else
            {
               ...
               {               	 
                   i4RetValue =  iReadDataFromDw9716(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
...
#if defined( COMMON_CAM_CAL_DRV)
EXPORT_SYMBOL(DW9716_EEPROM_Ioctl);
#else
