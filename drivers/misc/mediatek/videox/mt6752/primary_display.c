...
static void _primary_path_cmd_lock()
{
	mutex_lock(&(pgc->cmd_lock));
}

static void _primary_path_cmd_unlock()
{
...
static void _primary_path_vsync_lock()
{
	mutex_lock(&(pgc->vsync_lock));
}

static void _primary_path_vsync_unlock()
{
...
int _disp_primary_path_set_vfp(int enter)
{
    ...
    if(primary_display_is_video_mode())
   	{	
		...
		if(enter)
		{
			dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle_vfp, DDP_DSI_VFP_LP, &(lcm_param->dsi.vertical_vfp_lp));
		}else{
			dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle_vfp, DDP_DSI_VFP_LP, &(lcm_param->dsi.vertical_frontporch));
		}
...
int primary_display_save_power_for_idle(int enter, unsigned int need_primary_lock)
{
    ...
    if(1==disp_low_power_disable_ddp_clock)  //open
    {
        ...
        if((1==enter) && (primary_display_is_video_mode()==0))  // only for command mode
        {
            if(isIdlePowerOff==0)
            {
        ...
        else if((0==enter)&& (primary_display_is_video_mode()==0))  //close
        {
            if(isIdlePowerOff==1)
            {
    ...
    if(disp_low_power_remove_ovl==1)
    {
        ...
        {            
            if(enter)
            {               
                 DISPMSG("disp_low_power_ovl:remove. \n");
                 if(pgc->session_id==DISP_SESSION_DIRECT_LINK_MODE)
                 {
            ...
            {
                 DISPMSG("disp_low_power_ovl:add. \n");
                 if(pgc->session_id==DISP_SESSION_DECOUPLE_MODE)
                 {
	...
	{	
		...
		for(i = 0;i<DISP_INTERNAL_BUFFER_COUNT;i++)
		{
			...
			if(buf)
			{
				...
				dc_vAddr[i] = pSrc;
				DISPMSG("buf:0x%p, buf->list:0x%lx, buf->mva:0x%08x, buf->size:0x%08x, buf->handle:0x%p,dc_vaddr = 0x%lx\n",
				    buf, (unsigned long)(&buf->list), buf->mva, buf->size, buf->handle,dc_vAddr[i]);
...
static int _ovl_fence_release_callback(uint32_t userdata)
{
    ...
    if(primary_display_is_decouple_mode()==0 && primary_display_is_video_mode()==1 && gEnableOVLStatusCheck==1)
    {       
       ...
       {
           DISPMSG("ovl status before flush: 0x%x, 0x%x \n", ovl_status[0], ovl_status[1]);
           MMProfileLogEx(ddp_mmp_get_events()->ddp_abnormal_irq, MMProfileFlagPulse, ovl_status[0]|(ovl_status[1]<<16), DISP_MODULE_UNKNOWN);
           ret = -1;
       }
    }
    ...
    for (;;)
    {
        ...
        if(pgc->session_mode  == DISP_SESSION_DECOUPLE_MIRROR_MODE)
        {
            ...
            if (ret == 0){
                cmdqRecReset(cmdq_handle);
                _cmdq_insert_wait_frame_done_token_mira(cmdq_handle);
                cmdqBackupReadSlot(pgc->rdma_buff_info, 0, &(decouple_rdma_config.address));
                cmdqBackupReadSlot(pgc->rdma_buff_info, 1, &(decouple_rdma_config.pitch));
                _config_rdma_input_data(&decouple_rdma_config, pgc->dpmgr_handle, cmdq_handle);
                layer = disp_sync_get_output_timeline_id();
...
int primary_display_wait_for_vsync(void *config)
{
	...
	if(pgc->state == DISP_SLEEPED)
	{
		DISPCHECK("VSYNC DISP_SLEEPED Return\n");
